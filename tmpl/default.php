...
<p class="lead"> 
    <?php if(isset($docs[count ($docs)][1])):?>
        Nalezeno položek : <?=$docs[count ($docs)][1]?>
    <?php 
    if ($docs[count ($docs)][1] == 1) {
        $product_link = ('?option=com_virtuemart&view=productdetails&virtuemart_product_id='.($docs[0] ["virtuemart_product_id"]).'&virtuemart_category_id='.($docs[0] ["virtuemart_category_id"]));
